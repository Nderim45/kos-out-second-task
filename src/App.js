...
import { Paper, Tabs, Tab, Box, Typography, isMuiElement, Card } from "@material-ui/core";
import "./App.css";
    ...
    <div class="div-1">
      <Card square className='card'>
        ...
        {domainsItems.map(item => (
          ...
          {webHostItems.map(item => (
            content(item,value,1)
          ))}
          {domainsItems.map(item => (
            content(item,value,2)
          ))}
          {webHostItems.map(item => (
            content(item,value,3)
          ))}
          {domainsItems.map(item => (
            content(item,value,4)
          ))}
          {webHostItems.map(item => (
            content(item,value,5)
          ))}
          {domainsItems.map(item => (
            content(item,value,6)
          ))}
          {webHostItems.map(item => (
            content(item,value,7)
          ))}
          </Box>
      </Card>
    </div>
